...
$(document).ready(function () {
  $("#nav-item-edit_deposit_networks").css({
  ...
  $add_net_form.on("submit", function (e) {
    ...
    let add_deposit_net = () => {
      $.ajax({
        type: "POST",
        url: "/networks/add_deposit_network",
        data: self.serialize(),
        dataType: "json",
        timeout: 5000,
    ...
    common.manipulateAjax(add_deposit_net);
  });
  /*================================================== */
  /*=============== edit network ==================== */
  ...
  $edit_net_form.on("submit", function (e) {
    ...
    let edit_deposit_net = () => {
      $.ajax({
        type: "PUT",
        url: "/networks/edit_deposit_network",
        data: self.serialize(),
        dataType: "json",
        timeout: 5000,
        success: function (response) {
    ...
    common.manipulateAjax(edit_deposit_net);
  });
  /*=============== remove network ==================== */
  ...
  async function networkClickEvent(e) {
    ...
    let id = $(_this).children(".hidden_network_id").html().trim(),
      ...
      block_confirmations = await excludeSpanText(
        $(_this).children(".block_confirmations")
      ),
      minimum_deposit = await excludeSpanText(
        $(_this).children(".minimum_deposit")
      ),
      est_arrival = await excludeSpanText($(_this).children(".est_arrival")),
      our_deposit_address = await excludeSpanText($(_this).children(".our_deposit_address"));
    ...
    $edit_net_form
      .children("input[name = block_confirmations]")
      .val(block_confirmations);
    $edit_net_form
      .children("input[name = minimum_deposit]")
      .val(minimum_deposit);

    if ($(_this).has(".est_arrival")) {
      $edit_net_form.children("input[name = est_arrival]").val(est_arrival);
    }
    if ($(_this).has(".our_deposit_address")) {
      $edit_net_form.children("input[name = our_deposit_address]").val(our_deposit_address);
    }
  ...
  async function removeNetworkClickEvent(e) {
    ...
    let delete_deposit_net = () => {
      $.ajax({
        type: "DELETE",
        url: "/networks/delete_deposit_network",
